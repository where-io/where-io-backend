...
class RemoverLocalUsecaseImplTest {
    ...
    class QuandoSucesso {
        ...
        @Test
        @DisplayName("deve chamar removerLocalPorId com o id correto quando o id é válido")
        void deveChamarRemoverLocalPorIdComIdCorretoQuandoIdValido() {
            doNothing().when(localRepositoryPort).removerLocalPorId(ID_VALIDO);

            removerLocalUsecaseImpl.execute(ID_VALIDO);
